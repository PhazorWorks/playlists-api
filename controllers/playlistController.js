...
exports.status = async (req, res) => {
    ...
    let error;
    // get DB
    let db = initialization.getDB();
    // if db is connected
    if (db?.connection?.readyState !== 1) {
        status = false;
        error = "Database is not connected.";
    }
    // if we have an api key setup
...
exports.create = async (req, res) => {
    try {
        ...
        if (!user_id)
            return res
                .status(400)
                .json({success: false, message: "No user id was sent."});
        if (!tracks)
            return res
                .status(400)
                .json({success: false, message: "No tracks was sent."});
        if (!name)
            return res
                .status(400)
                .json({success: false, message: "No name was sent."});
    ...
    try {
        ...
        if (!query)
            return res.status(404).json({
                success: false,
                message: `playlist with name ${name} was not found.`,
            });
...
exports.list = async (req, res) => {
    try {
        ...
        if (!query)
            return res
                .status(404)
                .json({success: false, message: `No playlists was found.`});
...
exports.add = async (req, res) => {
    try {
        const {user_id, track, name} = req.body;
        if (!user_id)
            return res
                .status(400)
                .json({success: false, message: "No user id was sent."});
        if (!track)
            return res
                .status(400)
                .json({success: false, message: "No track was sent."});
        if (!name)
            return res
                .status(400)
                .json({success: false, message: "No name was sent."});
        ...
        query.tracks = query.tracks.concat(track);
        query.save();
...
exports.delete = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            success: true,
            message: `playlist with name ${name} was deleted.`,
        });
    } catch (e) {
        console.error(e);
        res.status(500).json({success: false, message: "Unknown error."});
...
exports.share = async (req, res) => {
    ...
    if (raw) {
        ...
        if (!id)
            return res
                .status(400)
                .json({success: false, message: "No id was sent."});
